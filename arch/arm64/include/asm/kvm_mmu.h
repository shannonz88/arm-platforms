 ...
 * where the kernel idmap uses more levels than the kernel runtime
 * page tables (as seen when the kernel is configured for 4k pages,
 * 39bits VA, and yet memory lives just above that limit, forcing the
 * idmap to use 4 levels of page tables while the kernel itself only
 * uses 3). In this particular case, it doesn't matter which side of
 * VA_BITS we use, as we're guaranteed not to conflict with anything.
 *
